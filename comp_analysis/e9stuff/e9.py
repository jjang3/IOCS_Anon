...
def e9_rewrite(input_name, patch_name):
    ...
    tainted_in_dir  = os.path.join(taint_dir, input_name) 
    tainted_in_file = os.path.join(tainted_in_dir, input_name+"_list.out")

    in_file         = os.path.join(in_bin_dir, input_name+".out")
    out_file        = os.path.join(out_bin_dir, input_name+".out")
